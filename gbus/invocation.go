...
func (dfi *defaultInvocationContext) ReturnToQueue(ctx context.Context, publishing *amqp.Publishing) error {
	if dfi.tx != nil {
		return dfi.bus.returnToQueue(ctx, dfi.tx, publishing)
	}
	return dfi.bus.ReturnToQueue(ctx, publishing)
}
